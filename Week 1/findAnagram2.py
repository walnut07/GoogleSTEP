...
def extractSubsetAnagramsOfWordFromDict(new_dictionary, new_word): # Extract subset anagrams of the word you want to search.
    ...
    for i in range(len(new_dictionary)):            # Iterate thru all the words in the dictionary
        isSubset = True
        for string, count in new_word.items():
            if string not in new_dictionary[i]:     # If the character isn't in a word in a dictionary, 
                isSubset = False                    # the word in a dictionary isn't a subset anagram.
            else: 
                if new_dictionary[i][string] > count: # If the word in a dictionary has more same characters than the word you wanna search anagrams of,
                    isSubset = False                  # the word in a dictionary isn't a subset anagram.
    ...
    print(subsetAnagrams)   # Print it to examins if it has correct subsetAnagrams.
